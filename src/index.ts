...
// perMessageDeflate is ON by default in `ws` — for a box whose whole job is
// forwarding a high-frequency stream of small JSON frames (a few hundred
// bytes each) with as little added latency as possible, compressing every
// single one costs CPU time that's worth more than the bandwidth it saves,
// especially on a throttled/shared-CPU fly.io machine under load from many
// concurrent symbols. That CPU cost is a plausible source of the recurring
// 500ms-2s tick-latency spikes seen in the Amsterdam backend's logs (a
// flat, purely-geographic delay would show up as a stable ~150-200ms, not
// spiky) — disabling it removes a deflate/inflate pass from every frame in
// both directions.
